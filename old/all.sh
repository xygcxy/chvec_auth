...
suf=$6
	...
	/root/bin/ffmpeg -i /var/www/chvec_auth/header.mp4 -vf "drawtext=fontsize=25:fontcolor=white: fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF: text=授权'$num':y=110:x=115: draw=lt(t\,5),drawtext=fontsize=25:fontcolor=white: fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF: text='$purpose':y=110:x=330: draw=lt(t\,5),drawtext=fontsize=25:fontcolor=white: fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF:text=有效期至'$date':y=110:x=885: draw=lt(t\,5)" -qscale 3 -y /var/www/chvec_auth/tmp/$num.mpg
	...
	/root/bin/ffmpeg -i $video_path -vf "movie=/var/www/chvec_auth/img/vec1.png, scale=100:30 [movie]; [in] [movie]overlay=main_w-overlay_w-10:main_h-overlay_h-10:enable=lt(mod(t\,40)\,5) [out]" -qscale 3 /var/www/chvec_auth/tmp/$ID.mpg
